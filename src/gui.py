...
def capitalize_sentence(sentence: str) -> str:
    """
    Напишите первую букву с заглавной буквы, чтобы обработать предложение, учитывая при этом аббревиатуру.

        :param sentence: Предложение, которое необходимо обработать.
        :return: Предложение после первой буквы пишется с заглавной буквы.
    """
...
def get_single_cnts(image_file, image_raw, all_masks):
    ...
    for m in range(len(mask_pics)):
        ...
        # Превратите полупрозрачные пиксели в прозрачные
        transparent_image[transparent_image[..., 3] < 255] = 0
        # Получите цвет всех полностью непрозрачных пикселей
        non_transparent_pixels = transparent_image[transparent_image[..., 3] == 255, :3]
        # Получите уникальный цвет всех непрозрачных пикселей
        unique_colors = np.unique(non_transparent_pixels, axis=0)
        # Для каждого цвета выделите соответствующий контур
        contour_list = []
        ...
        # ================Пройдите по каждой цветовой комбинации пузырькового текста================
        ...
        if isinstance(cp_bubble, list):
            # Пузырьки - это градиентные цвета
            color_bubble = ColorGradient(cp_bubble)
        else:
            # Пузырьки монохромные
        ...
        # ================Получите соответствующую пузырьковую текстовую маску================
        ...
        for c in range(len(contour_list)):
            ...
            # Нарисуйте залитое белым изображение одного контура на черном фоне
            bit_white_bubble = cv2.drawContours(black_background.copy(), [single_cnt.contour], 0, 255, -1)

            # Получаем изображение исходного изображения в диапазоне bit_white_bubble, остальные части белые
            ...
            # Путем умножения маски символов на маску контура получается изображение, содержащее только символы внутри контура
            letter_in_bubble = cv2.bitwise_and(letter_mask, letter_mask, mask=bit_white_bubble)
            # Получите координаты всех ненулевых пикселей
            px_pts = np.transpose(np.nonzero(letter_in_bubble))
            # Вычислите количество ненулевых пикселей
            px_area = px_pts.shape[0]
            # Получите координаты x и y всех ненулевых пикселей
            all_x = px_pts[:, 1]
            all_y = px_pts[:, 0]

            # Вычислите минимальные и максимальные координаты x и y и добавьте указанные padding
            min_x, max_x = np.min(all_x) - padding, np.max(all_x) + padding
            min_y, max_y = np.min(all_y) - padding, np.max(all_y) + padding

            # предел padding Последующий диапазон координат не выходит за границы исходного изображения
            min_x, min_y = max(min_x, 0), max(min_y, 0)
            max_x, max_y = min(max_x, image_raw.shape[1]), min(max_y, image_raw.shape[0])

            # Обрежьте прямоугольную область с отступом из изображения с белым фоном
...
class GUI(QMainWindow):
    ...
    def render_widget_text(self):
        ...
        for index, option in enumerate(self.task_names):
            ...
            # По умолчанию выбрана первая QRadioButton
            if index == 0:
    ...
    def a0_para(self):
        ...
        # основывать QGraphicsScene и QGraphicsView объект
        self.graphics_scene = QGraphicsScene(self)  # Графический объект сцены
        self.graphics_view = QGraphicsView(self)  # Объект графического представления
        # 设置渲染、优化和视口更新模式
        # 设置渲染抗锯齿
    ...
    def a3_docks(self):
        ...
        # ================Создать инструмент настройки================
        self.lb_setting = QLabel('Настройки перевода')
        ...
        # Создайте раскрывающийся список выбора языка
        ...
        # Добавьте выпадающий список в макет
        ...
        self.lb_text = QLabel('Совместимость с LabelPlus')
        ...
        # ================Инструмент создания слоя================
        self.lb_layer = QLabel('Обеспечить совместимость для PC')
    ...
    def _display_image(self, pixmap):
        ...
        # ================Очистите предыдущее изображение================
        if self.image_item:
    ...
    def open_image_in_ps(self, file_path):
        if sys.platform == 'win32':
            photoshop_executable_path = "D:\Программы\Adobe Photoshop 2022/Photoshop.exe"  # 请根据您的Photoshop安装路径进行修改
    ...
    def change_image(self, step):
        ...
        # Проверьте, находится ли текущий путь к изображению в списке отфильтрованных изображений
    ...
    def show_feedback_dialog(self):
        recipient_email = "balb31072@mail.ru"  # 替换为您的电子邮件地址
        subject = "Bug Report - Инструмент для перевода комиксов"
        body = "Пожалуйста, подробно опишите проблему, с которой вы столкнулись, здесь：\n\n\n\n версия программного обеспечения：1.0.0"
        mailto_url = f"mailto:{recipient_email}?subject={subject}&body={body}"

        try:
            webbrowser.open(mailto_url)
        except webbrowser.Error as e:
            QMessageBox.warning(self, "Неверно", f"Невозможно открыть почтовый клиент:{e}")

    def check_for_updates(self):
        # Здесь вы можете обновить свое программное обеспечение в режиме онлайн
        pass
    ...
    def step0_analyze_frames(self):
        ...
        for p, image_file in enumerate(self.image_list):
            ...
            if image_file.name in image_data:
                ...
                logger.info(
                    f"Цвет, который чаще всего появляется в цвете границы：{dominant_color}, Название цвета：{color_name}, {color_ratio=}")
    ...
    def step1_analyze_bubbles(self):
        ...
        for p, image_file in enumerate(self.image_list):
            ...
            ih, iw = image_raw.shape[0:2]
            # ================Информация о прямоугольной сетке================
            if image_file.name in image_data:
                frame_grid_strs = image_data[image_file.name]
            else:
                frame_grid_strs = [f'0,0,{iw},{ih}~0,0,{iw},{ih}']
            # ================Модель распознает текст, и текст отображается белым цветом================
            ...
            for c in range(len(color_patterns)):
                ...
                if len(single_cnts_ordered) >= 1:
                    ...
                    alpha = 0.1
                    # Создайте изображение на фоне исходного изображения с прозрачностью 10%.
                    ...
                    # Нарисуйте контуры на прозрачном изображении, для каждого контура используется свой цвет
        ...
        for i in range(len(self.image_list)):
            ...
            for c in range(len(single_cnts_ordered)):
                ...
                if GlobalConfig.SYSTEM in ['MAC', 'M1']:
                    ...
                    # Разделите распознанный текст на несколько строк
                    lines = recognized_text.splitlines()
                # Удалить пустые строки
                non_empty_lines = [line for line in lines if line.strip()]
                # Объединить непустые строки в одну строку
                cleaned_text = GlobalConfig.LINE_FEED.join(non_empty_lines)
                print(cleaned_text)
                print('-' * 24)

                # ================Добавить изображение в файл docx================
                cropped_image_pil = Image.fromarray(single_cnt.cropped_image)
                image_dpi = cropped_image_pil.info.get('dpi', (96, 96))[0]  # Получите значение dpi изображения, если информации о dpi нет, то используйте значение по умолчанию 96
                ...
                # ================Добавьте распознанный текст в файл docx================
    ...
    def step3_translate(self):
        target_language = 'ru' # 'zh-CN'

        # Открыть файл docx
        OCR_doc = Document(self.OCR_docx_path)

        # Прочитайте и соедините весь текст абзаца в документе
        full_text = []
        ...
        for line in simple_lines:
            # Проверьте, не превышает ли длина текущей строки после добавления ее в текущий блок максимальное количество символов
            if len(current_chunk) + len(line) + 1 > max_chars:  # Добавление 1 предназначено для учета символа новой строки
                chunks.append(current_chunk.strip())
                current_chunk = ""
            current_chunk += line + "\n"

        # Добавьте последний блок (если есть содержимое)
    ...
    def start_task(self, checked):
        # При использовании сигнала нажатия QPushButton для подключения к слоту будет передано логическое значение, указывающее, выбрана ли кнопка
        # Проверьте, какая QRadioButton выбрана, и выполните соответствующую задачу
        self.task_name = self.bg_step.checkedButton().text()
        self.task_ind = self.bg_step.checkedId()
        print(f'[{self.task_ind}]{self.task_name}')
        # Выполните логику вашей задачи здесь
        if self.image_folder is not None:
            ...
            if self.image_folder.exists():
                ...
                # Следующий шаг выбирается автоматически, если только текущий шаг не является последним
        ...
        # Обрабатывайте ожидающие события, чтобы пользовательский интерфейс оставался отзывчивым
        QApplication.processEvents()
    ...
    def closeEvent(self, event):
        # Если есть папка для открытия изображения, сохраните его в настройках программы
        if self.image_folder:
            self._settings.setValue('last_opened_folder', self.image_folder)
        else:
            self._settings.setValue('last_opened_folder', '')
        # Сохраните геометрию и информацию о состоянии окна в настройках программы
